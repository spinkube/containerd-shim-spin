...
impl SpinEngine {
    async fn wasm_exec_async(&self, ctx: &impl RuntimeContext) -> Result<()> {
        ...
        if let Some(components_env) = env::var(constants::SPIN_COMPONENTS_TO_RETAIN_ENV).ok() {
            let components = components_env.split(',').collect::<Vec<&str>>();
            locked_app = spin_app::retain_components(locked_app, &components, &[&validate_service_chaining_for_components]).context(
                "failed to resolve application with only [{components:?}] components retained",
            )?;
        }
